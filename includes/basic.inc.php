...
/**
 * Formats a string from the format ssssttt
 * into the format mmm:ss.ttt (or mmm:ss if $tsec is false)
 */
...
/**
 * Formats a string from the format ssssttt
 * into the format hh:mm:ss.ttt (or hh:mm:ss if $tsec is false)
 */
function formatTimeH($MwTime, $tsec = true) {
...
function http_get_file($url, $openonly = false) {
	...
	fwrite($fp, 'GET ' . $uri . $query . " HTTP/1.0\r\n" .
	            'Host: ' . $url['host'] . "\r\n" .
	            'User-Agent: XASECO2-' . XASECO2_VERSION . ' (' . PHP_OS . '; ' .
	                         $aseco->server->game . ")\r\n\r\n");
