...
// Formatting cleanup & ManiaLink popups by Xymph
...
Aseco::registerEvent('onBeginMap', 'match_beginmap');
Aseco::registerEvent('onEndMap', 'match_endmap');
...
// disabled because redundant with /admin readmaplist command - Xymph
...
// called @ onBeginMap
function match_beginmap($aseco, $map) {
...
/* disabled because redundant with /admin readmaplist command - Xymph
		...
		if (file_exists($filepath)) {
			...
			if (!$rtn) {
				...
				$message = 'Successfully set new Playlist: '.$MatchSettings['col_chat_highlite'].$filename.$MatchSettings['col_chat_default'].' with '.$MatchSettings['col_chat_highlite'].$cnt.$MatchSettings['col_chat_default'].' maps';
			}
			$aseco->addCall('ChatSendToLogin', array($aseco->formatColors($message), $admin->login));
...
// called @ onEndMap
function match_endmap($aseco, $info) {
	global $rasp;
	...
	$TeamPoints = array();
	$map = $info[1];
	$db_map_id = $aseco->getMapId($map['UId']);

	$sql = 'INSERT INTO match_main (trackID) VALUES (' . $db_map_id . ')';
	mysql_query($sql);
	...
	if ($MatchSettings['savefile']) {
		...
		$stgout = str_replace('{TRACK}', stripcolors($map['Name']), $stgout);
