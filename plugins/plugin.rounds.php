...
Aseco::registerEvent('onBeginMap', 'reset_rounds');
Aseco::registerEvent('onRestartMap', 'reset_rounds');
...
// called @ onSync, onBeginMap, onRestartMap
...
function report_round($aseco) {
	...
	if (!empty($round_times)) {
		...
		if ($aseco->settings['rounds_in_window'] && function_exists('send_window_message'))
