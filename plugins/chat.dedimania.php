 ...
 * Called by chat_dedinew, chat_dedilive, endMap & beginMap (plugin.dedimania.php).
	...
	if (!isset($dedi_recs) || ($total = count($dedi_recs)) == 0) {
		...
		for ($i = 0; $i < $total; $i++) {
			...
			if (isset($cur_record['NewBest']) && $cur_record['NewBest']) {
				...
				if (in_array($cur_record['Login'], $players) /* && $cur_record['Game'] == $aseco->server->getGame() */) {
...
function chat_dedinew($aseco, $command) {
	...
	show_dedirecs($aseco, $aseco->server->map->name, $aseco->server->map->uid, $dedi_db['Challenge']['Records'], $command['author']->login, 0, 0);
}  // chat_dedinew
...
function chat_dedilive($aseco, $command) {
	...
	show_dedirecs($aseco, $aseco->server->map->name, $aseco->server->map->uid, $dedi_db['Challenge']['Records'], $command['author']->login, 2, 0);
}  // chat_dedilive
...
function chat_dedipb($aseco, $command) {
	...
	for ($i = 0; $i < count($dedi_recs); $i++) {
		$rec = $dedi_recs[$i];
		if ($rec['Login'] == $login /* && $rec['Game'] == $aseco->server->getGame() */) {
			$score = $rec['Best'];
			$rank = $i;
			$found = true;
			break;
		...
		if ($found) {
			...
			if ($found) {
				...
				$message = formatText($dedi_db['Messages']['DIFF_RECORD'][0],
				                      $message1, $message2,
				                      ($aseco->server->gameinfo->mode == Gameinfo::STNT ?
				                       $diff : sprintf("%s%d.%03d", $sign, $sec, $ths)));
...
function chat_dedistats($aseco, $command) {
	...
	$header = 'Dedimania Stats: {#black}' . stripColors($aseco->server->map->name);
	$stats = array();
	...
	$stats[] = array();
	$stats[] = array('               {#black}$l[http://dedimania.com/tm2stats/?do=stat&RecOrder3=RANK-ASC&UId=' . $dedi_db['Challenge']['UId'] . '&Show=RECORDS]View all Dedimania records for this track$l');
