...
function init_playerpanels($aseco, $player) {
	...
	if (($panels = ldb_getPanels($aseco, $player->login)) &&
		...
		if ($panels['records'] != '') {
			...
			$player->panels['records'] = set_panel_bg($player->panels['records'], $player->panelbg);
		} else {
...
function chat_recpanel($aseco, $command) {
	...
	elseif ($command['params'] != '') {
		...
		elseif ($panel == 'default') {
			$player->panels['records'] = set_panel_bg($aseco->panels['records'], $player->panelbg);
			...
			// load new panel
			if ($paneldata = @file_get_contents($panel_file)) {
				$player->panels['records'] = set_panel_bg($paneldata, $player->panelbg);
				update_recpanel($aseco, $player, $player->panels['pb']);
...
function update_allrecpanels($aseco, $data) {
	...
	foreach ($aseco->server->players->player_list as &$player) {
		...
		if ($data) {
			$pb = $rasp->getPb($player->login, $aseco->server->map->id);
