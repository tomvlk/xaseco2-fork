...
Aseco::registerEvent('onEndMap', 'music_nextsong');
...
class Music {
	...
	function Music($settings) {
		...
		if (isset($settings['OVERRIDE_MAP'][0]))
			$this->override = (strtolower($settings['OVERRIDE_MAP'][0]) == 'true');
		else
			$this->override = false;
...
function chat_music($aseco, $command) {
	...
	elseif ($command['params'][0] == 'settings') {
		...
		// display ManiaLink message
		display_manialink($login, $header, array('Icons128x32_1', 'Sound'), $info, array(1.0, 0.23, 0.77), 'OK');
	}
...
function event_music($aseco, $answer) {
	...
	if ($action >= -4000 && $action <= -2101) {
		...
		$aseco->console('player {1} clicked command "/music {2}"',
		                $player->login, abs($action)-2100);

		// jukebox selected song
		$command = array();
		$command['author'] = $player;
		$command['params'] = abs($action)-2100;
