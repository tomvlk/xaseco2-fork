...
// Repair XASECO2 records table inconsistencies with rs_times table
...
// Updated Apr 2012 by Xymph <tm@gamers.org>
	...
	if (!mysql_select_db('xaseco2')) {
		echo "could not select\n";
		exit;
	}

	$query = 'SELECT id,name FROM maps ORDER BY id';
	$reschl = mysql_query($query);

	if (mysql_num_rows($reschl) > 0) {
		echo 'Selected maps: ' . mysql_num_rows($reschl) . "\n\n";
		...
		while ($rowchl = mysql_fetch_object($reschl)) {
			...
			$query = 'SELECT playerid,score,date FROM records WHERE MapID=' . $rowchl->id .
			         ' ORDER BY score,date LIMIT ' . $maxrecs;
			$resrec = mysql_query($query);

			$query = 'SELECT DISTINCT playerid,score FROM rs_times t1 WHERE mapid=' . $rowchl->id .
			         ' AND score=(SELECT MIN(t2.score) FROM rs_times t2 WHERE mapid=' . $rowchl->id .
			...
			if (mysql_num_rows($resrec) > 0) {
				...
				while ($rowrec = mysql_fetch_object($resrec)) {
					...
					if ($rowrec->playerid != $rowtms->playerid ||
						...
						$query = 'SELECT date,checkpoints FROM rs_times WHERE mapid=' . $rowchl->id .
...
//						printf("%3d : %32s\t-> rec %3d: %5d/%5d differs from %d/%5d\n", $rowchl->id, stripColors($rowchl->name), $n, $rowrec->playerid, $rowrec->score, $rowtms->playerid, $rowtms->score);
						...
						if (mysql_affected_rows() != 1) {
							...
							$query = "UPDATE records
							          ...
							          WHERE MapId=" . $rowchl->id . " AND PlayerId=" . $rowtms->playerid;
							$result = mysql_query($query);
